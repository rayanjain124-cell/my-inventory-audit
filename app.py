...
try:
    ...
    st.error("Missing library! Please ensure 'streamlit-barcode-reader' is in your requirements.txt")
...
def save_data(df, file=DATA_FILE): 
    # Force columns to string to prevent crashes
...
if user_role == "Host (Admin)":
    ...
    # Initial Setup: Host chooses their own key
    if not saved_admin_key:
        ...
        h_input = st.text_input("Enter Admin Key to Unlock", type="password")
    ...
    if st.session_state.get('is_host'):
        if not os.path.exists(DATA_FILE):
            ...
            session_code = st.text_input("Set Session Code for Team", "1234")
            ...
            st.metric("Shortage", len(df[df['Audit_Status'] == "Pending"]))
            
            # FINAL EXCEL REPORT WITH SHORT/EXCESS SHEETS
...
# ---------------- AUDITOR SECTION ----------------
    ...
    if not os.path.exists(DATA_FILE):
        st.warning("Audit is not active.")
    else:
        with open(CONFIG_FILE, 'r') as f: config = json.load(f)
        ...
        if st.session_state.get('is_auditor'):
            ...
            # BRAND FILTER FOR AUDITORS
            unique_brands = sorted(df_audit['Brand'].unique().tolist())
            selected_brand = st.selectbox("Brand Wise Filter (What's left?):", ["All Brands"] + unique_brands)
            
            view_df = df_audit[df_audit['Audit_Status'] == "Pending"]
            if selected_brand != "All Brands": view_df = view_df[view_df['Brand'] == selected_brand]
            st.info(f"Items remaining in {selected_brand}: {len(view_df)}")

            # DUAL SCANNING: TYPE OR CAMERA
            tab1, tab2 = st.tabs(["⌨️ Manual / Gun Scanner", "📷 Mobile Camera Scan"])
            scanned_code = ""
            with tab1: scanned_code = st.text_input("Type or Scan here", key="manual_scan")
            ...
            if scanned_code:
                ...
                target, m_type = (m_serial, "Serial No") if not m_serial.empty else (m_item, "Item No.") if not m_item.empty else (pd.DataFrame(), "")
                ...
                if not target.empty:
                    ...
                    if df_audit.at[idx, 'Audit_Status'] == "✅ Scanned":
                        ...
                        df_audit.at[idx, 'Matched_On'] = m_type # TRACK HOW IT WAS AUDITED
                        save_data(df_audit)
                        st.success(f"MATCH: {df_audit.at[idx, 'Product']}")
                else:
                    new_ex = pd.DataFrame([{"Product": "Excess Item", "Serial No": val, "Scanned_By": st.session_state.auditor_name, "Audit_Status": "EXCESS"}])
