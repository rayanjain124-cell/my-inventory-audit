...
def save_data(df, file=DATA_FILE): 
    try:
        # Prevent type errors by forcing everything to string
...
if user_role == "Host (Admin)":
    ...
    if st.session_state.get('is_host'):
        if not os.path.exists(DATA_FILE):
            ...
            session_code = st.text_input("Set Auditor Session Code (e.g. 1234)")
            ...
            # Final Multi-Sheet Excel Report
            ...
            st.download_button("📥 Download Final Excel Report", buffer.getvalue(), "Final_Audit_Report.xlsx")
...
# ---------------- AUDITOR SECTION ----------------
    ...
    if not os.path.exists(DATA_FILE):
        ...
        if st.session_state.get('is_auditor'):
            ...
            # Universal Scanning: Laptop/PC (Tab 1) or Mobile (Tab 2)
            tab1, tab2 = st.tabs(["⌨️ Manual / Gun Scanner", "📷 Mobile Camera"])
            code = ""
            with tab1: code = st.text_input("Scan/Type Here", key="man_input")
            with tab2: 
                if HAS_SCANNER:
                    code = streamlit_barcode_reader(key='cam_input')
                else:
                    st.error("Camera library failed. Use Manual tab.")
