...
# Optimized camera library for universal device support
try:
...
def save_data(df, file=DATA_FILE): 
    try:
        # Prevent crashes by ensuring all data is saved as string
...
user_role = st.sidebar.radio("Role", ["Host (Admin)", "Auditor (Scanner)"])
...
if user_role == "Host (Admin)":
    ...
    if not saved_admin_key:
        st.subheader("Step 1: Set Admin Password")
        new_key = st.text_input("Create secret Host password", type="password")
    ...
    if st.session_state.get('is_host'):
        if not os.path.exists(DATA_FILE):
            st.subheader("Step 2: Start Session")
            ...
            if file_main:
                try:
                    ...
                    df_main = df_raw.copy()
                    # Column Mapping: Item No (D), Brand (I), Category (M)
            ...
            st.metric("Pending Items", len(df[df['Audit_Status'] == "Pending"]))
...
# ---------------- AUDITOR SECTION ----------------
    ...
    if not os.path.exists(DATA_FILE):
        ...
        if st.session_state.get('is_auditor'):
            ...
            st.info(f"Left in {sel_brand}: {len(view_df)}")

            tab1, tab2 = st.tabs(["⌨️ Manual Entry / Gun", "📷 Camera Scan"])
            code = ""
            with tab1: code = st.text_input("Scan/Type Here", key="man_input")
            with tab2: 
                if HAS_SCANNER:
                    st.write("Camera Feed Active (Mobile Optimized)")
                    captured = camera_input_live()
                    if captured:
                        st.image(captured, width=200)
                        st.caption("Frame captured. For high-speed auditing, use Manual Entry tab with a Bluetooth scanner.")
                else:
                    st.error("Camera library failed to load.")
